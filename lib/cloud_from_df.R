cloud_from_df <- function(this.df, which.context, min.freq=0, size.breaks, save.to, note=NULL, standardize.font=TRUE, color=FALSE, title=FALSE){
  # this.df <- df %>% 
  ...
  } else warning(paste("No plot for", which.context, "because no words occur more than", min.freq, "times"))
}
