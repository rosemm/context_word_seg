...
<< document_prep, echo=FALSE, results=hide >>=
...
For this analysis, our approach was to use a list of key words that we expected would be associated with particular contexts – for example, milk was on the list for mealtime words, and wipe was on the list for diaper change words. The context lists were based on MCDI words, previous work describing infants' everyday activities (\cite{Place2011}; cite FAUSEY poster) and other models that have attempted to define contexts in infants' experience (\cite{Soderstrom2013}; \cite{Roy2012}), supplemented with some especially frequent content words from the corpus. The key word lists are available in Appendix~\ref{app:KeyWords}. Utterances in the corpus were then tagged for each context (not mutually exclusively) when those key words occurred, yielding 7 context-specific subcorpora. Word segmentation decisions based on syllable co-occurrence statistics were made within each context independently of the others, modeling zero generalization from one context to another on the part of infants. 
...
<< general_disc, child='gen_disc.Rnw' >>=
@


% - With this single line of code, Latex will print your entire bibliography!
