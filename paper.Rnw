...
source("/Users/TARDIS/Dropbox/library_addcites.R")
...
The original corpus \citep[available via CHILDES][]{MacWhinney2000} is transcribed in CHAT format, but because we were interested in the co-occurence of syllables we needed the phonetic transcription. 
...
For example, prosodic information, which has been demonstrated to be a useful cue to word boundaries for infants both on its own \citep{Johnson2009} and in combination with statistical phonological cues \citep{Shukla2007}, is not available to the model, potentially handicapping performance. 
...
For this analysis, our approach was to use a list of key words that we expected would be associated with particular contexts  --- for example, \emph{milk} was on the list for mealtime words, and \emph{wipe} was on the list for diaper change words. 
...
Utterances in the corpus were then tagged for each context (not mutually exclusively) when those key words occurred. 
Because not every utterance that occurs in a given context will contain context-specific words from the key word lists, we applied a smoothing procedure, including the two utterances immediately before and after a tagged utterance as well. 
This procedure yielded 7 context-specific subcorpora.
...
\label{app:KeyWords}
{\renewcommand{\arraystretch}{.6} % this changes the vertical spacing in the table below (see http://tex.stackexchange.com/questions/50332/vertical-spacing-of-a-table-cell)
<<key_words, echo=FALSE, results='asis' >>=
key_words <- read.csv("words by contexts.csv")
library(xtable)
print(xtable(key_words), include.rownames=FALSE, floating=FALSE)
@
