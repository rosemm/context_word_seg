...
write.table(df.hj.raw, file="contexts_files/contexts_HJ.txt", quote=F, col.names=T, row.names=F, append=F, sep="\t")
...
fun.version <- "8e1b6319ab44f" # fun.version refers to the current commit for data_processing_functions.r
```
...
aciss_function(fun.version, id="Sim_size_Korman", starts=1:50, iter=50, par_function_args)
...
sim_results <- rbind(size_skew, size_unif)
...
ggplot(filter(sim_results, sim=="size"), aes(x=N.utts, y=value)) +
  ...
  facet_wrap( ~ method + measure, scales="free_y", ncol=2) +
