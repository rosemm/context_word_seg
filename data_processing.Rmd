...
ggsave(filename=paste0("plots/topicmodeling/topic_prevalence_winutts", wn ,".png"), width=12, height=12, units="in")
```
...
colnames(thetas) <- paste0("topic_", 1:nK)
thetas <- cbind(out$meta, thetas)
df.temp <- left_join(df.temp, thetas, by=c( "child", "age_weeks", "wn.count" ))
df.stm <- df.temp %>%
  ungroup() %>%
  select(utt, orth, starts_with("topic_") )

write.table(df.stm, file=paste0("contexts_STM_winutts", wn, ".txt"), quote=F, col.names=T, row.names=F, append=F, sep="\t")

hist(unlist(thetas[,5:16]), breaks=50)
```
...
ggplot(filter(df.threshold.plot, measure=="mean_num_topics"), aes(x=threshold, y=value)) + 
  ...
  labs(y=NULL) +
  ggtitle(paste0("mean number of contexts per utterance\nutterances per document=", wn))
ggsave(filename=paste0("plots/topicmodeling/topic_thresholds_means_winutts", wn ,".png"), width=8, height=8, units="in")
...
ggplot(filter(df.threshold.plot, measure!="mean_num_topics"), aes(x=threshold, y=value, color=measure)) + 
  ...
  ggtitle(paste0("number of contexts per utterance\nutterances per document=", wn))
ggsave(filename=paste0("plots/topicmodeling/topic_thresholds_perc_winutts", wn ,".png"), width=12, height=8, units="in")

threshold <- .2 # set threshold for inclusion in a context
```
