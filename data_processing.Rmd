...
df_fam <- df %>% 
  extract(col=utt, into="fam", regex="(^[[:alpha:]]{2})", remove=FALSE) %>% 
  mutate(value=1) %>% 
  spread(key=fam, value=value, fill=0)

write.table(df_fam, file="contexts_files/contexts_Fam.txt", quote=F, col.names=T, row.names=F, append=F, sep="\t")
...
ggplot(size.data, aes(x=reorder(context, desc(n.utts)), y=n.utts)) +
  ...
  theme(axis.text.x = element_text(angle=330, vjust=1, hjust=0, size=20))
...
df_WL_bare <- get_from_https("https://raw.githubusercontent.com/rosemm/context_word_seg/master/contexts_files/contexts_WL.txt") 
df_WL_bare[df_WL_bare==1.5] <- 0 # erase the effects of the +-2 uterance smoothing

df_Fam <- get_from_https("https://raw.githubusercontent.com/rosemm/context_word_seg/master/contexts_files/contexts_Fam.txt") 
...
aciss_function(fun.version, id="nontexts_WL", starts=1:50, iter=200, par_function_args) 

par_function_args$dataframe <- df_Fam
aciss_function(fun.version, id="nontexts_Fam", starts=1:50, iter=200, par_function_args) 

par_function_args$dataframe <- df_STM
aciss_function(fun.version, id="nontexts_STM", starts=1:50, iter=200, par_function_args) 

par_function_args$dataframe <- df_HJ
aciss_function(fun.version, id="nontexts_HJ", starts=1:50, iter=200, par_function_args)

par_function_args$dataframe <- df_LDA
aciss_function(fun.version, id="nontexts_LDA", starts=1:50, iter=200, par_function_args)
...
par_function_args$embedding.rule <- TRUE
par_function_args$trisyl.limit <- TRUE
par_function_args$cutoff <- .75

par_function_args$dataframe <- df_WL
aciss_function(fun.version, id="nontexts_WL_freq", starts=1:50, iter=200, par_function_args) 

par_function_args$dataframe <- df_WL_bare
aciss_function(fun.version, id="nontexts_WL_bare_freq", starts=1:50, iter=200, par_function_args) 

par_function_args$dataframe <- df_Fam
aciss_function(fun.version, id="nontexts_Fam_freq", starts=1:50, iter=200, par_function_args) 
...
par_function_args$dataframe <- df_LDA
aciss_function(fun.version, id="nontexts_LDA_freq", starts=1:50, iter=200, par_function_args)

showStatus(makeRegistry(id = "nontexts_WL_freq")); showStatus(makeRegistry(id = "nontexts_WL_bare_freq")); showStatus(makeRegistry(id = "nontexts_Fam_freq")); showStatus(makeRegistry(id = "nontexts_STM_freq")); showStatus(makeRegistry(id = "nontexts_HJ_freq")); showStatus(makeRegistry(id = "nontexts_LDA_freq"))
...
par_function_args <- list(dataframe=df_WL_old, 
                          dict=dict, 
                          consider.freq=FALSE,
                          ...
                          quiet=TRUE,
                          fun.version=fun.version)
aciss_function(fun.version, id="nontexts_WL_expand", starts=1:50, iter=100, par_function_args)
...
###########################################
par_function_args$consider.freq <- FALSE
par_function_args$embedding.rule <- FALSE
par_function_args$trisyl.limit <- FALSE
...
par_function_args$dataframe <- df_Fam
context_Fam_results <- par_function(par_function_args)
context_Fam <- left_join(context_Fam_results$stat.results, context_Fam_results$continuity.measures$continuity.stats, by=c("nontext"="context"))
...
par_function_args$dataframe <- df_WL
context_WL_freq_results <- par_function(par_function_args)
...
par_function_args$dataframe <- df_WL_bare
context_WL_bare_freq_results <- par_function(par_function_args)
context_WL_bare_freq <- context_WL_bare_freq_results$stat.results
context_WL_bare_freq_seg <- context_WL_bare_freq_results$MI85.seg.results

par_function_args$dataframe <- df_Fam
context_Fam_freq_results <- par_function(par_function_args)
context_Fam_freq <- context_Fam_freq_results$stat.results
context_Fam_freq_seg <- context_Fam_freq_results$MI85.seg.results

par_function_args$dataframe <- df_STM
context_STM_freq_results <- par_function(par_function_args)
...
par_function_args$dataframe <- df_LDA
context_LDA_freq_results <- par_function(par_function_args)
...
par_function_args$dataframe <- df_HJ
context_HJ_freq_results <- par_function(par_function_args)
...
context_WL_bare_freq$method <- "WL_bare"
context_Fam_freq$method <- "Fam"
context_STM_freq$method <- "STM"
context_LDA_freq$method <- "LDA"
context_HJ_freq$method <- "HJ"

context_all_freq <- rbind(context_WL_freq, context_WL_bare_freq, context_Fam_freq, context_STM_freq, context_LDA_freq, context_HJ_freq)
...
context_Fam_freq_seg$method <- "Fam"
context_STM_freq_seg$method <- "STM"
context_LDA_freq_seg$method <- "LDA"
context_HJ_freq_seg$method <- "HJ"

context_seg <- rbind(context_WL_freq_seg, context_Fam_freq_seg, context_STM_freq_seg, context_LDA_freq_seg, context_HJ_freq_seg)
...
nontext_WL_bare <- read_batch(dir="nontexts_WL_bare_freq-files")
nontext_Fam <- read_batch(dir="nontexts_Fam_freq-files")
...
# combine all the context results into one data frame
nontext_WL$method <- "WL"
nontext_WL_bare$method <- "WL_bare"
nontext_Fam$method <- "Fam"
nontext_STM$method <- "STM"
nontext_LDA$method <- "LDA"
nontext_HJ$method <- "HJ"

nontext_results_freq <- rbind(nontext_WL, nontext_WL_bare, nontext_Fam, nontext_STM, nontext_LDA, nontext_HJ)
...
par_function_args$dataframe <- df[1:100,] # only taking the first three columns, and 100 rows, for speed
quick_check <- par_function(par_function_args)

par_function_args$dataframe <- df[ , 1:3] # only taking the first three columns
...
par_function_args$embedding.rule <- TRUE
par_function_args$trisyl.limit <- TRUE
par_function_args$verbose <- TRUE
par_function_args$cutoff <- .75
...
for(i in 1:length(cutoffs)){
  message(cutoffs[i])
  par_function_args$cutoff <- cutoffs[i]
  this.output <- par_function(par_function_args)
...
global_data_freq_results <- par_function(par_function_args)
...
syls_Fam <- syllable_dist(df_Fam, dict, method="Family", save.to="syl_dists")
