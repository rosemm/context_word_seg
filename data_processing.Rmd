...
max(table(df.temp$wn.count)) # if this is greater than wn, error!
...
doc.data <- group_by(df.temp, wn.count) %>%
  ...
  ungroup() %>%
  dplyr::select( -wn.count) # drop the wn.count variable, since it's no longer needed

##########################
# STM
##########################
# prep
processed <- textProcessor(doc.data$documents, metadata = doc.data)
