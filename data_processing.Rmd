...
df.temp <- na.omit(df.temp) # delete the extra empty rows made as a buffer between recordings

stopifnot(wn >= max(table(df.temp$wn.count))) # if this is greater than wn, error!

# collapse utterances from within each wn-utterance window
doc.data <- group_by(df.temp, wn.count) %>%
...
top.topic.words(lda$topics, 10, by.score=TRUE) ## Get the top words in the cluster
```

```{r lda_doc_loadings}
lda.loadings <- t(lda$document_sums) / colSums(lda$document_sums)
colnames(lda.loadings) <- paste0("topic_", 1:nK)

lda.loadings <- cbind(out$meta, lda.loadings)
df.temp.lda <- left_join(df.temp, lda.loadings, by=c( "child", "age_weeks", "wn.count" ))
df.lda <- df.temp.lda %>%
  ungroup() %>%
  left_join(df, by=c("utt", "orth")) %>% # adding the phon column back in
...
```{r lda_threshold}
df.threshold <- gather(df.lda, key="topic", value="loading", starts_with("topic_"))

thresholds <- seq(0,.75,.025) # what thresholds to try
...
df.threshold.plot <- df.threshold %>% 
  group_by(threshold) %>% 
...
df.threshold.plot$measure <- factor(df.threshold.plot$measure, 
...
ggplot(filter(df.threshold.plot, measure=="mean_num_topics"), aes(x=threshold, y=value)) + 
  geom_line() + 
  scale_y_continuous(breaks=0:12) +
  scale_x_continuous(breaks=round(seq(min(thresholds), max(thresholds), by=.05), 2)) +
  geom_line(y=1, linetype=2) +
  labs(y=NULL) +
  ggtitle(paste0("LDA: mean number of contexts per utterance\nutterances per document=", wn))
ggsave(filename=paste0("plots/topicmodeling/topic_thresholds_LDA_means_winutts", wn ,".png"), width=8, height=8, units="in")

ggplot(filter(df.threshold.plot, measure!="mean_num_topics"), aes(x=threshold, y=value, color=measure)) + 
  geom_line() + 
  scale_x_continuous(breaks=round(seq(min(thresholds), max(thresholds), by=.05), 2)) +
  labs(y="Percent of total utterances") +
  ggtitle(paste0("LDA: number of contexts per utterance\nutterances per document=", wn))
ggsave(filename=paste0("plots/topicmodeling/topic_thresholds_LDA_perc_winutts", wn ,".png"), width=12, height=8, units="in")
```
...
fit.cont <- stm(out$documents, # the documents
           ...
           data = out$meta, 
           init.type = "Spectral")
fit.prev <- stm(out$documents, # the documents
           out$vocab, # the words
           K = nK, # number of topics
           prevalence =~ child,
           max.em.its = 75, 
           data = out$meta, 
           init.type = "Spectral")
fit.both <- stm(out$documents, # the documents
           out$vocab, # the words
           K = nK, # number of topics
           prevalence =~ child, 
           content =~ child,
...
fits <- list(fit.null=fit.null, fit.cont=fit.cont, fit.prev=fit.prev, fit.both=fit.both)
...
for(f in 1:length(fits)){
  ...
  pdf(paste0("plots/topicmodeling/topics_winutts", wn, "_",names(fits)[[f]], ".pdf"))
  plot.STM(fit, type = "summary", main = paste0("Topics\n", names(fits)[[f]])) 
  dev.off()
  
  print(labelTopics(fit))
  print(findThoughts(fit, texts=out$meta$documents, topics=1:nK, n=3))
  
  pdf(paste0("plots/topicmodeling/convergence_winutts", wn, "_",names(fits)[[f]], ".pdf"))
  plot(fit$convergence$bound, type = "l",
     ylab = "Approximate Objective",
     main = paste0("Convergence\n", names(fits)[[f]]))
  dev.off()
  
  pdf(paste0("plots/topicmodeling/topic_net_winutts", wn, "_",names(fits)[[f]], ".pdf"))
  # topicCorr(fit) # lots of output :)
  plot(topicCorr(fit), main = paste0("Topic correlations\n", names(fits)[[f]]))
  dev.off()
  
  pdf(paste0("plots/topicmodeling/topic_heatmap_winutts", wn, "_",names(fits)[[f]], ".pdf"))
  c <- topicCorr(fit)$cor # just the correlations between topics
  print(round(c, 2))
...
est <- estimateEffect(1:nK ~ child, fit, meta=out$meta)
...
```{r stm_doc_loadings}
stm.loadings  <- as.data.frame(fit$theta)
colnames(stm.loadings) <- paste0("topic_", 1:nK)
stm.loadings <- cbind(out$meta, stm.loadings)
df.temp.stm <- left_join(df.temp, stm.loadings, by=c( "child", "age_weeks", "wn.count" ))
df.stm <- df.temp.stm %>%
  ungroup() %>%
  left_join(df, by=c("utt", "orth")) %>% # adding the phon column back in
  select(utt, orth, phon, starts_with("topic_") ) 
...
```{r stm_threshold}
df.threshold <- gather(df.stm, key="topic", value="loading", starts_with("topic_"))
...
ggplot(filter(df.threshold.plot, measure=="mean_num_topics"), aes(x=threshold, y=value)) + 
  ...
  geom_line(y=1, linetype=2) +
  labs(y=NULL) +
  ggtitle(paste0("STM: mean number of contexts per utterance\nutterances per document=", wn))
ggsave(filename=paste0("plots/topicmodeling/topic_thresholds_STM_means_winutts", wn ,".png"), width=8, height=8, units="in")
...
ggplot(filter(df.threshold.plot, measure!="mean_num_topics"), aes(x=threshold, y=value, color=measure)) + 
  ...
  scale_x_continuous(breaks=round(seq(min(thresholds), max(thresholds), by=.05), 2)) +
  labs(y="Percent of total utterances") +
  ggtitle(paste0("STM: number of contexts per utterance\nutterances per document=", wn))
ggsave(filename=paste0("plots/topicmodeling/topic_thresholds_STM_perc_winutts", wn ,".png"), width=12, height=8, units="in")
```
