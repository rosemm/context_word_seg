...
master_doc_prop$utt <- master_doc_count$utt  # to add back in the non-numeric columns
...
plot.data <- data.frame(n.utts=counts, 
                        context=factor(names(counts), levels=names(counts)))
plot.data$over.med <- ifelse(plot.data$n.utts > median(counts), "yes", "no")
ggplot(plot.data, aes(x=context, y=n.utts, fill=over.med)) +
...
write.table(df.hj.con, file="contexts_HJ.txt", quote=F, col.names=T, row.names=F, append=F, sep="\t")
