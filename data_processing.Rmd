...
# update_doc <- UpdateDoc(master_doc, criterion = 10, unique.coders=TRUE)
...
ggsave("plots/HJ/uttsper_context.png", width=12, height=5, units="in")
ggplot(count.data, aes(x=context, y=n.utts, fill=over.med)) +
  geom_bar(stat = "identity", show.legend = F) +
  geom_text(aes(label=n.utts), position=position_dodge(width=0.9), vjust=-0.25)+
  scale_fill_manual(values = c("yes" = "darkgreen","no" = "grey")) +
  ...
  scale_y_log10() +
  theme(axis.text.x = element_text(angle=330, vjust=1, hjust=0)) 
ggsave("plots/HJ/uttsper_context_logtrans.png", width=12, height=5, units="in")
...
write.table(df.hj.misc, file="contexts_files/contexts_HJ.txt", quote=F, col.names=T, row.names=F, append=F, sep="\t")
# df.hj.con <- read.table("contexts_files/contexts_HJ.txt", header=1, sep="\t", stringsAsFactors=F, quote="", comment.char ="") 
```

```{r HJ_threshold}
df.hj.con <- read.table("contexts_files/contexts_HJ_raw.txt", header=1, sep="\t", stringsAsFactors=F, quote="", comment.char ="") 
...
fun.version <- "19595e8a8d4ee85" # fun.version refers to the current commit for data_processing_functions.r
```
...
par_function_args$dataframe <- df_WL
aciss_function(fun.version, id="Sim_cutoff_WL", starts=starts, iter=500, par_function_args) 

par_function_args$dataframe <- df_STM
aciss_function(fun.version, id="Sim_cutoff_STM", starts=starts, iter=500, par_function_args) 

par_function_args$dataframe <- df_HJ
aciss_function(fun.version, id="Sim_cutoff_HJ", starts=starts, iter=500, par_function_args)

par_function_args$dataframe <- df_LDA
aciss_function(fun.version, id="Sim_cutoff_LDA", starts=starts, iter=500, par_function_args)
...
aciss_function(fun.version, id="Sim_size_Korman", starts=1:50, iter=100, par_function_args)
...
# to correct mistakes:
# killJobs(makeRegistry(id = "Sim_cutoff_LDA"),findRunning(makeRegistry(id = "Sim_cutoff_LDA")))
