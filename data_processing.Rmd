...
batch.conf[3] <- "mail.done = 'first+last'"
batch.conf[length(batch.conf)+1] <-"mail.to = '<rosem@uoregon.edu>'"
batch.conf[length(batch.conf)+1] <-"mail.from = '<rosem@uoregon.edu>'"
...
fun.version <- "64ce775dad31f28a" # fun.version refers to the current commit for data_processing_functions.r
```
...
kable(filter(tests.contexts, criterion=="MI85")[,-2], digits=3)
```
