...
#PBS -M rosem@uoregon.edu
#PBS -m abe
## merge standard error and output
#PBS -j oe
## direct streams to our logfile
#PBS -o <%= log.file %>
#PBS -l walltime=<%= resources$walltime %>,nodes=1:ppn=12,vmem=<%= resources$memory %>
...
fun.version <- "b297e2f85b7862" # fun.version refers to the current commit for data_processing_functions.r
```
...
aciss_function(fun.version, id="Sim_size_Korman", starts=1:100, iter=5, par_function_args) # this takes much longer to run, so need to use fewer iterations so ACISS doesn't kill the job
...
ggplot(filter(sim_results, sim=="size" & method=="Korman"), aes(x=N.utts, y=value)) +
  geom_jitter(alpha=.2, width = 0.4) +
  geom_point(context_results, aes(color=method, shape=method), size=4, show.legend=TRUE) +
  facet_wrap( ~ measure, scales="free", ncol=2) +
  theme(text = element_text(size=30), axis.ticks = element_blank()) +
  labs(x="Number of utterances", y=NULL) 
