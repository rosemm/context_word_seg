...
df.hj.raw <- left_join(df[,1:3], master_doc_prop, by="utt" ) 
write.table(df.hj.raw, file="contexts_HJ_raw.txt", quote=F, col.names=T, row.names=F, append=F, sep="\t")
...
ggsave("plots/HJ/uttspercontext.png", width=12, height=5, units="in")
ggplot(count.data, aes(x=context, y=n.utts, fill=over.med)) +
  geom_bar(stat = "identity", show.legend = F) +
  scale_fill_manual(values = c("yes" = "darkgreen","no" = "grey")) +
  ...
  scale_y_log10() +
  theme(axis.text.x = element_text(angle=330, vjust=1, hjust=0)) 
ggsave("plots/HJ/uttspercontext_logtrans.png", width=12, height=5, units="in")

##########################################
# for contexts with fewer than the median number of utterances, replace with "misc"
##########################################
master_doc_count.misc <- master_doc_count %>% 
  gather(key="context", value="value", -utt, -file, -UttNum) %>% 
  left_join(count.data, by="context") 
master_doc_count.misc$context <- ifelse(master_doc_count.misc$over.med=="no", "misc", master_doc_count.misc$context)  
master_doc_count.misc <- master_doc_count.misc %>% 
  select(utt, context, value) %>% 
  group_by(utt, context) %>% 
  summarise(value=sum(value)) %>% 
  spread(key=context, value=value)
master_doc_prop.misc <- master_doc_count.misc %>% 
  ungroup() %>%
  dplyr::select(-utt) %>%
  mutate(total=rowSums(., na.rm=FALSE)) %>% 
  mutate_each(funs(./total)) %>% 
  select(-total)
master_doc_prop.misc$utt <- master_doc_count.misc$utt  # to add back in the non-numeric utt column

df.hj.misc <- left_join(df[,1:3], master_doc_prop.misc, by="utt" ) 

write.table(df.hj.misc, file="contexts_HJ.txt", quote=F, col.names=T, row.names=F, append=F, sep="\t")
# df.hj.con <- read.table("contexts_HJ.txt", header=1, sep="\t", stringsAsFactors=F, quote="", comment.char ="") 
```
...
write.table(df.hj.bin, file=paste0("contexts_HJ_bin.txt"), quote=F, col.names=T, row.names=F, append=F, sep="\t")
```
...
LDA.con <- read.table(paste0("contexts_LDA_winutts", wn, ".txt"), header=1, sep="\t", stringsAsFactors=F, quote="", comment.char ="")
LDA.bin <- read.table(paste0("contexts_LDA_bin_winutts", wn, ".txt"), header=1, sep="\t", stringsAsFactors=F, quote="", comment.char ="")

HJ.con <- read.table("contexts_HJ.txt", header=1, sep="\t", stringsAsFactors=F, quote="", comment.char ="") 
...
## run on aciss
This code is all run on ACISS (the UO high performance cluster), not on your local machine. 
...
fun.version <- "a2ff73b8b0c83f" # fun.version refers to the current commit for data_processing_functions.r
```
...
par_function_args[[1]] <- df_WL
aciss_function(fun.version, id="nontexts_WL", starts=1:20, iter=100, par_function_args) 

par_function_args[[1]] <- df_STM
aciss_function(fun.version, id="nontexts_STM", starts=1:20, iter=100, par_function_args) 

par_function_args[[1]] <- df_HJ
aciss_function(fun.version, id="nontexts_HJ", starts=1:20, iter=100, par_function_args)

par_function_args[[1]] <- df_LDA
aciss_function(fun.version, id="nontexts_LDA", starts=1:20, iter=100, par_function_args)
...
par_function_args[[1]] <- df_WL
context_WL <- par_function(par_function_args)

par_function_args[[1]] <- df_STM
context_STM <- par_function(par_function_args)

par_function_args[[1]] <- df_LDA
context_LDA <- par_function(par_function_args)

par_function_args[[1]] <- df_HJ
context_HJ <- par_function(par_function_args)
```
...
context_ests <- context_results %>% 
  unite(col="analysis", method, context, remove=FALSE) %>% 
  select(criterion, analysis, measure, value) %>% 
...
nontext_results <- nontext_results %>% 
  gather( key="measure", value="value", recall, precision) %>% 
  unite(col="criterion", stat, cutoff, sep="_", remove=FALSE) %>% 
  rename(nontext=nontext) 

nontext_results$measure <- as.factor(nontext_results$measure)
...
global_results <- colMeans(global.data$MI85$seg.results[,3:4], na.rm=T)
global_results <- as.data.frame(t(global_results)) # make it a data.frame
global_results$stat <- "MI"
global_results$cutoff <- .85
global_results$N.utts <- global.data$N.utterances
global_results$N.segd.units <- unique(global.data$MI85$seg.results$N.segd.units)
global_results$N.words <- global.data$streams$N.tokens
global_results$TTR <- global.data$streams$N.types/global.data$streams$N.tokens
...
ggplot(full_results, aes(x=context, y=value)) +
  ...
  facet_wrap( ~ method + measure, scales="free_x", ncol=2) +
...
global_results <- data.frame(measure=NA, criterion=NA, global.est=NA)
global_results[1,] <- c("recall",    "TP85", colMeans(global.data$TP85$seg.results[, 3:4], na.rm=T)[1])
global_results[2,] <- c("precision", "TP85", colMeans(global.data$TP85$seg.results[, 3:4], na.rm=T)[2])
global_results[3,] <- c("recall",    "MI85", colMeans(global.data$MI85$seg.results[, 3:4], na.rm=T)[1])
global_results[4,] <- c("precision", "MI85", colMeans(global.data$MI85$seg.results[, 3:4], na.rm=T)[2])
global_results$global.est <- as.numeric(global_results$global.est)

full.results <- left_join(full.results, global_results)
```
